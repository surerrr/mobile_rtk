...
EXPORT int input_rt17(raw_t *Raw, unsigned char Data)
{
    ...
    */
    ...
    /*
    | We do indeed have an entire packet.我们确实有一个完整的数据包
    | Check the packet checksum.检查包校验和
    */
    ...
    if (PacketBuffer[2] == RAWDATA)
    {
        ...
        如果这是RAWDATA数据包序列中的第一个RAW数据包，
        请确保它是第一页，而不是中间的数据包。
        如果不是第一页，则跳过它并继续从流中读取，
        直到找到一个从页面开始的数据包。
        否则，请确保它是与最后一个数据包相同的数据包请求的一部分，
        即它的页码是按顺序排列的。
        */
    ...
    for (i = n = 0; (i < nsat) && (i < MAXOBS); i++)
    {
        ...
        else /* Expanded Format */
        {
            ...
            */
            ...
            {
                /*
                | We can't use the IODE flags in this context.我们不能在这种情况下使用IODE标志
                | We already have slip flags and don't need slip counters.
                */
                p++; /* U1 IODE, Issue of Data Ephemeris */
                p++; /* U1 L1 cycle slip roll-over counter */ 
