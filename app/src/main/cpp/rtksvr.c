...
//                obs.data[obs.n].D[2] = ObsData.GalD2_t[j];
            ...
            /* rtk positioning */
            rtksvrlock(svr);
            rtkpos(&svr->rtk,obs.data,obs.n,&svr->nav);//TODO 定位计算
            rtksvrunlock(svr);
...
*          int     *strs    I  stream types (STR_???流的接口类型，tcpip,netrip)
...
*          int     *format  I  input stream formats (STRFMT_???数据类型，rtcm3,rtcm2)
...
* return : status (1:ok 0:error)
*-----------------------------------------------------------------------------*/
extern int rtksvrstart(rtksvr_t *svr, int cycle, int buffsize, int *strs,  //TODO rtksvrstart
