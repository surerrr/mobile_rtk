...
static int connect_nb(socket_t sock, struct sockaddr *addr, socklen_t len)
{
    ...
    fcntl(sock,F_SETFL,flag|O_NONBLOCK);      //yuan  hanshu
    //fcntl(sock,F_SETFL,O_NONBLOCK);         //edit by liuyi
    if (connect(sock,addr,len)==-1) {
        err=errsock();
...
static int gentcp(tcp_t *tcp, int type, char *msg)
{
   ...
   // test=socket(AF_UNIX,SOCK_STREAM,0);
    /* generate socket */
    if ((tcp->sock=socket(AF_INET,SOCK_STREAM,0))==(socket_t)-1) {      //edit liuyi
    //if ((tcp->sock=socket(AF_UNIX,SOCK_STREAM,0))==(socket_t)-1) {
    ...
    tcp->addr.sin_family=AF_INET;           //edit liuyi
    //tcp->addr.sin_family=AF_UNIX;
